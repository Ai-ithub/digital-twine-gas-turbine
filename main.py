...
class DataGenerator:
    ...
    def _generate_base_signals(self, time: np.ndarray, chunk_size: int) -> Dict[str, np.ndarray]:
    ...
    def _calculate_derived_values(self, signals: Dict) -> Dict[str, np.ndarray]:
        ...
        signals['temperature_out'] = signals['temperature_in'] * (signals['pressure_ratio'] ** ((self.GAMMA - 1) / (self.GAMMA * signals['efficiency'])))
        signals['power_consumption'] = signals['flow_rate'] * self.CP * (signals['temperature_out'] - signals['temperature_in']) / signals['efficiency']
        return signals
    ...
    def _label_status(self, signals: Dict) -> Dict[str, np.ndarray]:
        ...
        signals['status'] = np.where(
            (signals['vibration'] < normal_threshold) & (signals['power_consumption'] < power_threshold) & (signals['efficiency'] > 0.80),
            "Normal",
            np.where(
                ((signals['vibration'] >= normal_threshold) & (signals['vibration'] < imbalance_threshold)) |
                ((signals['power_consumption'] >= power_threshold) & (signals['power_consumption'] < fault_power_threshold)),
                "Imbalance", "Fault"
            )
        )
        return signals

    def _create_dataframe(self, time: np.ndarray, device_id: int, signals: Dict, chunk_size: int) -> pd.DataFrame:
        """Assembles the final DataFrame from all generated signals."""
        return pd.DataFrame({
            "Time": time,
            "Device_ID": f"SGT-400-{device_id:02d}",
            "Pressure_In": signals['pressure_in'],
            "Temperature_In": signals['temperature_in'] - 273.15, # Convert to Celsius
            "Flow_Rate": signals['flow_rate'],
            "Pressure_Out": signals['pressure_out'],
            "Temperature_Out": signals['temperature_out'] - 273.15, # Convert to Celsius
    ...
    def _inject_imperfections(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        noise_std = {'Pressure_In': 0.2, 'Temperature_In': 1.0, 'Flow_Rate': 0.2, 'Vibration': 0.1, 'Efficiency': 0.01}
    ...
    def generate_chunk(self, start_time: int, chunk_size: int, device_id: int) -> pd.DataFrame:
    ...
    def run_generation_loop(self):
        ...
        for device_id in range(1, self.num_devices + 1):
            ...
            for chunk_index in range(num_chunks):
                ...
                print(f"✅ Chunk {chunk_index+1}/{num_chunks} for device SGT-400-{device_id:02d} saved.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--records", type=int, default=1_000_000, help="Total records to generate per device.")
    parser.add_argument("--chunksize", type=int, default=500_000, help="Number of records per CSV file (chunk).")
    parser.add_argument("--output", type=str, default="compressor_datasets", help="Directory to save the output CSV files.")
