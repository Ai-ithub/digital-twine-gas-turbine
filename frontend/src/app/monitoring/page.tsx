...
// import { IntensityGraph } from "../../components/Intensity_graph" // adjust the path as needed
...
export default function MonitoringPage() {
  ...
  // Select the latest sensor data from redux store
  const realTimeData = useSelector((state: RootState) => state.sensor.latest);

  // You can have your alerts state locally or from redux as well
  ...
  useEffect(() => {
    // Connect to websocket and dispatch data on mount
  ...
  // Render your UI using realTimeData from redux (make sure default values if empty)
  if (!realTimeData) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            ...
            <div className="text-3xl font-bold text-blue-400 mb-2">
              {realTimeData.pressure_in?.toFixed(1) ?? "-"} bar
            </div>
            <div className="w-full bg-gray-700 rounded-full h-2">
              <div
                className="bg-blue-500 h-2 rounded-full transition-all duration-300"
                style={{
                  width: `${((realTimeData.pressure_in ?? 0) / 120) * 100}%`,
                }}
              />
            </div>
          </div>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            ...
            <div className="text-3xl font-bold text-red-400 mb-2">
              {realTimeData.temperature_in?.toFixed(1) ?? "-"}°C
            </div>
            <div className="w-full bg-gray-700 rounded-full h-2">
              <div
                className="bg-red-500 h-2 rounded-full transition-all duration-300"
                style={{
                  width: `${((realTimeData.temperature_in ?? 0) / 100) * 100}%`,
                }}
              />
            </div>
          </div>
