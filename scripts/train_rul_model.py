...
def load_and_preprocess_data(config: dict):
    ...
    feature_cols = [col for col in numeric_cols if col not in [config['target_column'], 'Time']]
...
def train_rul_model(config: dict):
    ...
    X, y = create_sequences(df[feature_cols].values, df[config['target_column']].values, config['window_size'])
    ...
    try:
        ...
        input_tensor = tf.keras.Input(shape=(n_timesteps, n_features), name="input_layer")
        output_tensor = model(input_tensor)
        functional_model = tf.keras.Model(inputs=input_tensor, outputs=output_tensor, name="rul_model")

        input_signature = [tf.TensorSpec([None, n_timesteps, n_features], tf.float32, name="input_layer")]
        
        # Convert the new functional model
        model_proto, _ = tf2onnx.convert.from_keras(functional_model, input_signature, opset=13)
