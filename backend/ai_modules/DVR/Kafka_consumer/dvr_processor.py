...
class DVRProcessor:
    ...
    def apply_rule_based_checks(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['rule_pressure_in'] = df['Pressure_In'].between(self.RULE_THRESHOLDS['PRESSURE_IN_MIN'], self.RULE_THRESHOLDS['PRESSURE_IN_MAX'])
        df['rule_temperature_in'] = df['Temperature_In'].between(self.RULE_THRESHOLDS['TEMPERATURE_IN_MIN'], self.RULE_THRESHOLDS['TEMPERATURE_IN_MAX'])
        df['rule_flow_rate'] = df['Flow_Rate'].between(self.RULE_THRESHOLDS['FLOW_RATE_MIN'], self.RULE_THRESHOLDS['FLOW_RATE_MAX'])
        df['rule_pressure_diff'] = (df['Pressure_Out'] - df['Pressure_In']).between(self.RULE_THRESHOLDS['PRESSURE_DIFF_MIN'], self.RULE_THRESHOLDS['PRESSURE_DIFF_MAX'])
        df['rule_efficiency'] = df['Efficiency'].between(self.RULE_THRESHOLDS['EFFICIENCY_MIN'], self.RULE_THRESHOLDS['EFFICIENCY_MAX'])
        df['rule_vibration'] = df['Vibration'].between(self.RULE_THRESHOLDS['VIBRATION_MIN'], self.RULE_THRESHOLDS['VIBRATION_MAX'])
        df['rule_ambient_temp'] = df['Ambient_Temperature'].between(self.RULE_THRESHOLDS['AMBIENT_TEMP_MIN'], self.RULE_THRESHOLDS['AMBIENT_TEMP_MAX'])
        df['rule_power'] = df['Power_Consumption'].between(self.RULE_THRESHOLDS['POWER_MIN'], self.RULE_THRESHOLDS['POWER_MAX'])
        ...
        df['rule_temp_roc'] = df['delta_temperature'] < self.RULE_THRESHOLDS['TEMP_ROC_LIMIT']
        df['rule_vib_roc'] = df['delta_vibration'] < self.RULE_THRESHOLDS['VIB_ROC_LIMIT']
        df['rule_pressure_roc'] = df['delta_pressure'] < self.RULE_THRESHOLDS['PRESSURE_ROC_LIMIT']
    ...
    def apply_pca_check(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        threshold = np.percentile(reconstruction_error, self.PCA_RECONSTRUCTION_PERCENTILE)
    ...
    def apply_wls_model(self, df: pd.DataFrame) -> sm.regression.linear_model.RegressionResultsWrapper:
        ...
        """
        df_clean = df.dropna(subset=['Pressure_In', 'Temperature_In', 'Flow_Rate', 'Efficiency', 'Power_Consumption'])
        ...
        weights = 1.0 / (residual_std ** 2 + 1e-9) # Add epsilon to avoid division by zero
