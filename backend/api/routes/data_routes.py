...
def get_live_data():
    ...
    try:
        with InfluxDBClient(url=config['INFLUXDB_URL'], token=config['INFLUXDB_TOKEN'], org=config['INFLUXDB_ORG']) as client:
            ...
            results = [
                {"time": record.get_time().isoformat(), "field": record.get_field(), "value": record.get_value()}
    ...
    # CHANGED: More specific error handling
        ...
        return jsonify({"error": "Service unavailable: Could not connect to InfluxDB"}), 503
...
def get_all_data():
    ...
    # CHANGED: More specific error handling
        ...
        return jsonify({"error": "Service unavailable: Could not connect to database"}), 503
