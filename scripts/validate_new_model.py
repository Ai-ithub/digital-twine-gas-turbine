...
def load_test_data(model_type: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        ...
        logging.warning(
            "Test dataset not found. Generating mock data for demonstration."
        )
        # Create dummy data with the required size and columns
        if model_type == "RTM":
            features = np.random.rand(1000, len(config.RTM_FEATURE_COLUMNS)).astype(
                np.float32
            )
            ...
            features = np.random.rand(1000, len(config.RUL_MODEL_FEATURES)).astype(
                np.float32
            )
...
def evaluate_model(
    model_uri: str, X_test: np.ndarray, Y_true: np.ndarray, model_type: str
) -> dict:
    """
    ...
    try:
        ...
        if model_type == "RTM":
            ...
            logging.info(
                f"RTM Metrics: F1={metrics['F1_score']:.4f}, Recall={metrics['Recall']:.4f}"
            )
...
def validate_and_promote(model_type: str, candidate_version: int):
    ...
    logging.info(
        f"--- Starting validation for {model_name} (Candidate v{candidate_version}) ---"
    )
    ...
    try:
        ...
        logging.warning(
            f"No existing Production model found for {model_name}. Auto-promoting candidate."
        )
        return "PROMOTE"
    ...
    if model_type == "RTM":
        ...
        if (
            f1_improvement >= PROMOTION_THRESHOLD_RTM
            and recall_improvement >= PROMOTION_THRESHOLD_RTM
        ):
            logging.info(
                f"SUCCESS: RTM Candidate shows significant improvement (F1: +{f1_improvement:.4f}, Recall: +{recall_improvement:.4f})"
            )
            return "PROMOTE"
        else:
            logging.info(
                f"FAIL: RTM Candidate did not meet promotion threshold (F1 change: {f1_improvement:.4f}, Recall change: {recall_improvement:.4f})"
            )
            return "REJECT"
        ...
        relative_reduction = (
            mae_reduction / prod_metrics["MAE"] if prod_metrics["MAE"] else 0
        )

        if relative_reduction >= PROMOTION_THRESHOLD_RUL:
            logging.info(
                f"SUCCESS: RUL Candidate shows significant improvement. MAE reduced by {relative_reduction:.2%} (Absolute: {mae_reduction:.2f} days)"
            )
            return "PROMOTE"
        else:
            logging.info(
                f"FAIL: RUL Candidate MAE reduction ({relative_reduction:.2%}) is below {PROMOTION_THRESHOLD_RUL:.2%} threshold."
            )
            return "REJECT"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Automated ML Model Validation and Promotion Script."
    )
    parser.add_argument(
        "--model_type",
        type=str,
