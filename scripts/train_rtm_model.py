...
class MockConfig:
    ...
    RTM_FEATURE_COLUMNS = [
        ...
        "Vibration_roll_mean", "Power_Consumption_roll_mean",
        "Vibration_roll_std", "Power_Consumption_roll_std",
    ]
config = MockConfig()
...
def train_rtm_model(cfg: dict):
    ...
    with mlflow.start_run() as run:
        ...
        df["Vibration_roll_mean"] = df["Vibration"].rolling(window=cfg["window_size"]).mean()
        df["Power_Consumption_roll_mean"] = df["Power_Consumption"].rolling(window=cfg["window_size"]).mean()
        df["Vibration_roll_std"] = df["Vibration"].rolling(window=cfg["window_size"]).std()
        df["Power_Consumption_roll_std"] = df["Power_Consumption"].rolling(window=cfg["window_size"]).std()
        ...
        y = df["Status"].apply(lambda x: cfg["normal_label"] if x == "Normal" else cfg["anomaly_label"]) 
        ...
        # 5. Train Model
        model = RandomForestClassifier(n_estimators=cfg["n_estimators"], random_state=cfg["random_state"], n_jobs=-1)
        ...
        logger.info(f"✅ Logged F1-score (Anomaly): {f1:.4f}, Recall (Anomaly): {recall:.4f}")
        ...
        logger.info(f"✅ ONNX model saved to MLflow artifacts folder.")
        ...
        mlflow.log_artifact(os.path.join(temp_dir, "rtm_scaler_mean.npy"), "scaler_params")
        mlflow.log_artifact(os.path.join(temp_dir, "rtm_scaler_scale.npy"), "scaler_params")
        logger.info(f"✅ Scaler parameters saved to MLflow artifacts folder.")
        ...
        logger.info(f"✅ Sklearn model saved for interpretability.")

        # 8. Register the Model (We register the ONNX model)
        # We need an input example for the signature
        input_example = X_test_scaled[:1].astype(np.float32) # Use scaled data for input example
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"Error: Data file not found. Please ensure the path is correct: {e}")
        logger.error("If running from 'scripts/' folder, make sure '../datasets/MASTER_DATASET.csv' exists.")