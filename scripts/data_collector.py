...
def fetch_influxdb_data(query_api: QueryApi, days_ago: int = 30) -> pd.DataFrame:
    ...
    try:
        ...
        df = df.rename(columns={"_time": "Timestamp"}).drop(columns=['result', 'table'], errors='ignore')
        df['Timestamp'] = pd.to_datetime(df['Timestamp'])
        
        logging.info(f"✅ Successfully fetched {len(df)} rows of time-series data from InfluxDB.")
...
def fetch_mysql_labels(start_time: dt.datetime) -> pd.DataFrame:
    ...
    try:
        ...
        if not df.empty:
            logging.info(f"✅ Successfully fetched {len(df)} maintenance events from MySQL.")
            df['Timestamp'] = pd.to_datetime(df['Timestamp'])
            return df.set_index('Timestamp').sort_index()
        else:
        ...
        logging.error("Ensure MySQL container is running and environment variables are set.")
        return pd.DataFrame()


def run_data_collector(days_ago: int = 30, output_path: str = "data/retraining_dataset.parquet"):
    """
    ...
    logging.info(f"✅ Training dataset successfully created and saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    run_data_collector(days_ago=365, output_path="data/training_data/retraining_dataset_pdm.parquet")